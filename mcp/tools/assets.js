...
export function registerAssetTools(server, { api, notifyMutation }) {
  ...
  server.registerTool('list_library_assets', {
    ...
    inputSchema: {
      type: z.enum(['image', 'mesh', 'brush', 'tree', 'vfx', 'building']).optional()
  ...
  server.registerTool('list_asset_tags', {
    ...
    inputSchema: {
      ...
      type: z.enum(['image', 'mesh', 'brush', 'tree', 'vfx', 'building']).optional().describe('Vocabulary only: count tags on this asset type alone.')
    },
    annotations: { readOnlyHint: true }
  ...
  server.registerTool('find_assets_by_tags', {
    ...
    inputSchema: {
      ...
      type: z.enum(['image', 'mesh', 'brush', 'tree', 'vfx', 'building']).optional().describe('Only assets of this type'),
  ...
  server.registerTool('import_library_assets', {
    ...
    inputSchema: {
      ...
      assetType: z.enum(['image', 'mesh', 'brush', 'tree', 'vfx', 'building']).optional().describe('Force the asset type for every file (otherwise inferred from each extension)')
    }
