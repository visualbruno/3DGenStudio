...
const BAKE_OPTIONS = {
  ...
  align_source: z.boolean().default(true).describe('Put the source into the same space as the target before baking. A bake casts rays from the target onto the source, so a source that sits elsewhere returns BLANK texels wherever the two stop overlapping — and one pivot move between producing the target and picking the source is enough to cause it. Re-centres a source that is the same size as the target, and rescales one whose bounding box is the target box times the SAME factor on all three axes — the case you get when the low-poly was simplified outside the Mesh Editor while the textured high-poly came back normalised to a unit box. A source whose axes disagree about that factor is a different object and is never moved; that case is reported in stats.alignment (mode "skipped-scale", with the per-axis ratios) instead.'),
...
export function registerMeshToolTools(server, { api, notifyMutation }) {
  ...
  server.registerTool('transfer_rig', {
    ...
    return {
      ...
      // The uniform factor the source had to be scaled by to sit on the target,
      // or 0 when it was already the right size.
      rescaled_by: stats.rescaled,
